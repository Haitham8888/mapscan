...
# Optional SQLite population DB support
# Set env var POP_DB to point to a sqlite file, otherwise default to data/population.db
POP_DB_PATH = os.environ.get('POP_DB') or os.path.join(os.path.dirname(__file__), 'data', 'population.db')


def _get_db_connection():
	"""Return sqlite3.Connection or None if DB file not present."""
	try:
...
def query_population_by_ids(region_id=None, city_id=None, district_id=None):
	...
	try:
		...
		if district_id:
			try:
				cur.execute('SELECT pop_m, pop_f, pop_total FROM population WHERE district_id = ? LIMIT 1', (district_id,))
				row = cur.fetchone()
				if row:
					pop_m = float(row['pop_m'] or 0)
					pop_f = float(row['pop_f'] or 0)
					pop_total = float(row['pop_total'] or (pop_m + pop_f))
					return {'population_male': pop_m, 'population_female': pop_f, 'population_total': pop_total}
			except Exception:
				pass
		if city_id:
			try:
				cur.execute('SELECT pop_m, pop_f, pop_total FROM population WHERE city_id = ? LIMIT 1', (city_id,))
				row = cur.fetchone()
				if row:
					pop_m = float(row['pop_m'] or 0)
					pop_f = float(row['pop_f'] or 0)
		...
		if region_id:
			try:
				cur.execute('SELECT pop_m, pop_f, pop_total FROM population WHERE region_id = ? LIMIT 1', (region_id,))
				row = cur.fetchone()
				if row:
					pop_m = float(row['pop_m'] or 0)
					pop_f = float(row['pop_f'] or 0)
